/* Watermain pressure monitoring and control system
...
*
* Modifications to the original software by Bob Jessup can be located by searching for the string REJ
* 
* List of modifications:
*   1. Pressure monitoring detects water leak based on duration of reduced pressure over adjustable defined time limit. 
*      Standby mode disables shutoff to allow for known excess water flow such as long showers by your teenager, etc.
*
*   2. Apply simultaneous standby timer and flow timer tracking to allow monitoring flow during standby periods
*   
*   3. Apply adaptive threshhold to prevent premature watermain cutoff
*****************************/
  ...
  int pressureThreshhold = 65;      // water pressure below this threshhold activates timer which determines when valve could be closed.
  unsigned long flowTimerFirst = 0; // time marker when water was first turned on.
  unsigned long flowTimer = 0;      // latest time marker water has been flowing
  int lastFlowIntervalSec = 0;      // difference between above flow timers in seconds tracks accumulated time water has been below pressure threshhold
  #define LAST_FLOW_INTERVAL_TOPIC "watermain/last_flow_interval_sec"   // topic used to publish flow interval  

  // variables which support standby monitoring to override valve closure criteria
  int standby = 0;                 // normal operation = 0, standby = time in minutes to disable flow monitoring while showering, etc.
  int standbyInit = 0;             // preserves the initial standby valus as standby timer is decreased
  unsigned long priorStandby = 0;  // tracks standby as it decreases minute by minute
  unsigned long standbyTimerFirst = 0;  //intial timer value in msec
  unsigned long standbyTimer = 0;  //standby timer in msec which is compared to standbyTimerFirst to calculate time elapsed
  #define STANDBY_TIMER_MINUTES_TOPIC "watermain/standby_timer_minutes"  // topic used to publish current standby timer in minutes

  // Adaptive threshhold variables
  float pmax = 0;                 // max pressure
  float pmin = 200;
  float T1 = 0;                   // threshhold 1
  float T2 = 0;
  float T = 0;
  int n = 0;
  int flowtime = 0;
  int leakyPeakSec = 600;
  float t1Delta = 1.0;  // 0 if default to pressureThreshhold above else delta psi to indicate flow has started
  float t2Delta = -4;  // 0 if default to equation else delta psi rise to indicate flow has stopped
  #define MAXPRESSURE_TOPIC "watermain/pmax"  
  #define MINPRESSURE_TOPIC "watermain/pmin"  
  #define T1_TOPIC "watermain/t1"  
  #define T2_TOPIC "watermain/t2"  
  #define T_TOPIC "watermain/t"
  #define N_TOPIC "watermain/ncount"  
  #define FLOWTIME_TOPIC "watermain/flowtime"  

  
  int valveOverride = 0;  // Manual override
  #define VALVE_OVERRIDE_TOPIC "watermain/valve_override"  
  int leakDetected = 0;
  #define LEAK_DETECTED_TOPIC "watermain/leak_detected"  
...
#define WIFI_PASSWORD SECRET_WIFI_PASSWORD    //  <<<<<<<  REPLACE WITH YOUR CREDENTIALS
#endif
...
#define VALVE_ROTATION_TIME_MS 15000 //12000 // 10000                 // time required for valve to open/close - relays are only active long enough for the valve to rotate
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  // *********************//////////////////////////////////////////////////////////////////////////////////////
  // Additional mqtt commands REJ begin
  // ********************* 
  // Additional valid commands:
  //  standby/<new_value>                     - assigns a new value, but does not save to NVM   
  //  flow_interval_limit_sec/<new_value>     - assigns a new value, but does not save to NVM  
  //  pressure_threshhold/<new_value>        - assigns a new value, but does not save to NVM  
  //  t1_delta
  //  t2_delta
  //  leaky_peak_sec
  //  valve_override                          - manual override disables automatic valve control
                                
  if (strstr(topic, "standby"))             // standby = 1 if standby mode, standby = 0 for normal operation
  {
    cmdValid = true;
    //if ((strcmp(msg, "0.0") == 0) || (strcmp(msg, "1.0") == 0))
    //{
      Serial.printf("standby set to %s\n", msg);
      standby = atoi(msg);
      if (standby>0){
        standby++;            //add one to allow for minute calculation being integer 
        standbyInit = standby;
        priorStandby = standby;
        standbyTimerFirst = millis();
        sprintf(msg, "%d", standby);
        mqttClient.publish(STANDBY_TIMER_MINUTES_TOPIC, msg, true);    // publish updated standby duration
      }  
      
      //reset parms to prevent premature valve shutoff after publishing latest interval
      flowTimerFirst = 0; 
      if (lastFlowIntervalSec>=1){
        sprintf(msg, "%d", lastFlowIntervalSec);        
        mqttClient.publish(LAST_FLOW_INTERVAL_TOPIC, msg, true); 
        lastFlowIntervalSec = 0;
      }
    //}
    //else
    //  Serial.println("Invalid standby value");
  }

  if (strstr(topic, "flow_interval_limit_sec")) // flow limit in seconds
  {
    cmdValid = true;
    if (atoi(msg) >= 1)
    {
      Serial.printf("flow_interval_limit_sec set to %s\n", msg);
      flowIntervalLimitSec = atoi(msg);
    }
    else
      Serial.println("Invalid flowLimitIntervalSec value");
  }
  
  if (strstr(topic, "pressure_threshhold")) // pressure threshhold
  {
    cmdValid = true;
    if (atoi(msg) >= 1)
    {
      Serial.printf("pressureThreshhold set to %s\n", msg);
      pressureThreshhold = atoi(msg);
    }
    else
      Serial.println("Invalid pressureThreshhold value");
  }

  if (strstr(topic, "t1_delta")) // pressure threshhold
  {
    cmdValid = true;
    if (atof(msg) >= 0)
    {
      Serial.printf("t1Delta set to %s\n", msg);
      t1Delta = atof(msg);
    }
    else
      Serial.println("Invalid t1Delta value");
  }

  if (strstr(topic, "t2_delta")) // pressure threshhold
  {
    cmdValid = true;
    //if (atof(msg) >= 0)
    //{
      Serial.printf("t2Delta set to %s\n", msg);
      t2Delta = atof(msg);
    //}
   // else
   //   Serial.println("Invalid t2Delta value");
  }

  if (strstr(topic, "leaky_peak_sec")) 
  {
    cmdValid = true;
    if (atoi(msg) >= 0)
    {
      Serial.printf("leakyPeakSec set to %s\n", msg);
      leakyPeakSec = atoi(msg);
    }
    else
      Serial.println("Invalid leakyPeakSec value");
  }

    if (strstr(topic, "valve_override")) 
  {
    cmdValid = true;
    if (atoi(msg) >= 0)
    {
      Serial.printf("valveOverride set to %s\n", msg);
      valveOverride = atoi(msg);

    //sprintf(msg, "%u", flowtime);
    mqttClient.publish(VALVE_OVERRIDE_TOPIC, msg);      
    }
    else
      Serial.println("Invalid valveOverride value");
  }

  if (strstr(topic, "leak_detected")) 
  {
    cmdValid = true;
//    if (atoi(msg) >= 0)
//    {
      ...
      if (valveOverride ==0){
        ...
        applyValveState(valveState, true);                         //close the valve         
      }
//      valveOverride = atoi(msg);
 //   }
 //   else
...
void loop()
{
  ...
  if (opParams.pressureInstalled == 1)
  {
    ...
    // *********************//////////////////////////////////////////////////////////////////////////////////////
    // Additional code REJ begin:
    // Pressure monitoring detects water leak based on duration of reduced pressure below threshhold over time limit and 
    // shuts off water valve if not in standby mode (standby > 0).
    //
    // If in standby mode, decrements the standby timer ea minute until it reaches 0 (standby = 0).
    // NOTE - if standby initial value is set to a value less than flow duration limit, it has no effect.  standby should
    //        be set to a higher value than flowIntervalLimitSec.
    //        For example, standby = 1200 (seconds), flowIntervalLimitSec = 180 would allow for 3 minutes flow normally,
    //        and 20 minutes flow during standby mode while someone may be showering, etc.).
    //
    
    delay(1000);                                                  // temporary mqtt throttle                                                  
    if(pressureThreshhold - medianPressure >=0)
    {
      // 1. water is flowing - manage flow timer and send flow duration via MQTT
      flowTimer = millis();
      if(flowTimerFirst==0){
        flowTimerFirst = flowTimer;
      }
      lastFlowIntervalSec = fabs(flowTimer-flowTimerFirst)/1000;  // update flow duration
      sprintf(msg, "%d", lastFlowIntervalSec);
      mqttClient.publish(LAST_FLOW_INTERVAL_TOPIC, msg, true);    // publish updated duration
    }else{
      // 2. water not flowing - reset the flow start timer
      flowTimerFirst = 0;                                         
    }

    if (standby>0)
    {
      // 3. System is in standby mode - decrement standby timer 
      standbyTimer = millis();
      standby = standbyInit-(fabs(standbyTimer-standbyTimerFirst)/60000);  // update standby duration in minutes
      if(standby<=0){
        standby=0;
      }
      
      // a. publish updated standby duration if minute counter has changed
      if(standby!=priorStandby){                                  
        sprintf(msg, "%d", standby);
        mqttClient.publish(STANDBY_TIMER_MINUTES_TOPIC, msg, true);    
        priorStandby = standby;
      }  
    }

    if ((standby == 0)&&(flowTimerFirst>0))
    {
      // 4. standby is off and water is flowing - close the valve if flow time exceeds the limit and reset the timer
      if ( (lastFlowIntervalSec - flowIntervalLimitSec) >=0)
      {  
//REJ 231126:        
if (valveOverride == 0){
//
  //to do - provide option to send message to HA instead of closing automatically. 
        valveState = CLOSE_VALVE;
        applyValveState(valveState, true);                         //close the valve 
        flowTimerFirst = 0;                                        // reset the timers
}
      }      
    }
    

    //5. Adaptive threshhold calculation
    //a. region A processing.  Region A is when there is no flow, i.e., psi exceeds threshhold T1.
    if (medianPressure > T1){
      if (medianPressure >= pmax){
        pmax = medianPressure;
        T1 = calcT1();
        n=0;
        //T2 = T1;
        //pressureThreshhold = T1;
      }else{
        n++;
        if(n >= leakyPeakSec){
          pmax = pmax-.25;
          if (pmax < medianPressure){
            pmax = medianPressure;
          }
          T1 = calcT1();
          n=0;
...
T=T1;
      ...
      pmin=T2;
      flowtime=0;
    }else{                // end of region A processing
   

      //b. region C processing.  Region C is when there has been flow recently, but has been shut off. i.e., psi transitions from below to above threshhold T2.
      //   This causes recalculation of T1.
      if (medianPressure >= T2){
        if (medianPressure >= pmax){
          pmax = medianPressure;
          T1 = calcT1();
          n=0;
 ...
 T=T1;      
        ...
        //c. region B processing.  Region B is when there is currently flow.
         if (medianPressure < pmin){
            pmin = medianPressure;
            T2 = calcT2();
            
        }
 //pressureThreshhold=T2;
 T=T2;
        pmax = T2;
        n=0;
        flowtime++;
    ...
    sprintf(msg, "%.2f", pmax);
    mqttClient.publish(MAXPRESSURE_TOPIC, msg);
    sprintf(msg, "%.2f", pmin);
    mqttClient.publish(MINPRESSURE_TOPIC, msg);
    sprintf(msg, "%.2f", T1);
    mqttClient.publish(T1_TOPIC, msg);
    sprintf(msg, "%.2f", T2);
    mqttClient.publish(T2_TOPIC, msg);
    sprintf(msg, "%.2f", T);
    mqttClient.publish(T_TOPIC, msg);
    sprintf(msg, "%u", n);
    mqttClient.publish(N_TOPIC, msg);      
    sprintf(msg, "%u", flowtime);
    mqttClient.publish(FLOWTIME_TOPIC, msg);      
      
  }   // end if (opparms.pressureInstalled = 1)
}     // loop() ends here
...
{   
  ...
  //force min T2 = pressureThreshhold floor
  if (T2 < pressureThreshhold){
    T2 = pressureThreshhold;   
  }
